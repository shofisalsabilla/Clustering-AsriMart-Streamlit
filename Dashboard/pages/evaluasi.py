...
def show():
    ...
    # ── ROW 1: PCA & Bar Chart (BERDAMPINGAN) ─────────
    ...
    with col_pca:
        ...
        for i, cat in enumerate(df_pca['Kategori'].unique()):
            ...
            ax.scatter(df_pca.loc[mask, 'PC1'], df_pca.loc[mask, 'PC2'], color=PALETTE[i % len(PALETTE)], label=cat, alpha=0.7)
        ax.scatter(c_pca[:, 0], c_pca[:, 1], c='white', s=150, marker='X', edgecolors='black', label='Centroid')
        ax.set_title('Visualisasi K-Means (PCA)', color='white')
        ax.tick_params(colors='#b0c4de'); ax.grid(True, linestyle='--', alpha=0.3)
    ...
    with col_bar:
        ...
        ax.tick_params(colors='#b0c4de'); ax.grid(axis='x', linestyle='--', alpha=0.3)
        st.pyplot(fig); plt.close(fig)

    st.markdown("---")

    # ── ROW 2: Distribusi & Top 10 (BERDAMPINGAN) ─────────
    ...
    with col_dist:
        ...
        fig, ax = plt.subplots(figsize=(8, 5))
        fig.patch.set_facecolor('#0d1b2a'); ax.set_facecolor('#0d1b2a')
        ax.pie(dist.values, labels=dist.index, autopct='%1.1f%%', colors=PALETTE[:len(dist)], textprops={'color': 'white'})
    ...
    with col_top:
        ...
        ax.invert_yaxis()
        ax.set_title('10 Barang Terlaris', color='white')
        ax.tick_params(colors='#b0c4de'); ax.grid(axis='x', linestyle='--', alpha=0.3)
        st.pyplot(fig); plt.close(fig)
