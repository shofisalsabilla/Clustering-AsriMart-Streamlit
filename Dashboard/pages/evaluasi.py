...
def show():
    ...
    with col_pca:
        ...
        ax.set_title('Cluster K-Means (PCA)', color='white')
        st.pyplot(fig)
        plt.close(fig)

    with col_bar:
        st.markdown("<div class='section-title'>📊 Bar Chart Rata-rata Qty</div>", unsafe_allow_html=True)
        st.caption("Perbandingan rata-rata volume penjualan antar kategori.")
    ...
    with col_top:
        ...
        st.caption("Daftar produk dengan total penjualan tertinggi.")
