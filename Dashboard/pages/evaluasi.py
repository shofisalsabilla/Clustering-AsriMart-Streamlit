...
def show():
    ...
    <div class='main-header'>
        ...
        <p style='margin:4px 0 0; color:#b0c4de; font-size:0.9rem;'>
            Analisis performa model clustering dan distribusi data produk
        </p>
    </div>
    ...
    # 1. PCA & Bar Chart (BERDAMPINGAN)
    col_pca, col_bar = st.columns(2)
    
    with col_pca:
        st.markdown("<div class='section-title'>🔵 Scatter Plot PCA</div>", unsafe_allow_html=True)
        st.caption("Visualisasi pemisahan cluster dalam ruang 2D untuk melihat efektivitas pengelompokan.")
        ...
        for i, cat in enumerate(df_pca['Kategori'].unique()):
            ...
            ax.scatter(df_pca.loc[mask, 'PC1'], df_pca.loc[mask, 'PC2'], label=cat, alpha=0.7)
        ax.set_title('Visualisasi K-Means (PCA)', color='white')
    ...
    with col_bar:
        ...
        st.caption("Perbandingan rata-rata volume penjualan antar kategori untuk profil cluster.")
    ...
    # 2. Distribusi & Top 10 (BERDAMPINGAN)
    col_dist, col_top = st.columns(2)

    with col_dist:
        st.markdown("<div class='section-title'>📦 Distribusi Cluster</div>", unsafe_allow_html=True)
        st.caption("Proporsi persentase jumlah produk di setiap kelompok.")
        dist = df_clustered['Kategori'].value_counts()
        fig, ax = plt.subplots(figsize=(6, 4))
        fig.patch.set_facecolor('#0d1b2a')
        ax.pie(dist.values, labels=dist.index, autopct='%1.1f%%', 
               colors=[PALETTE[i%len(PALETTE)] for i in range(len(dist))], textprops={'color': 'white'})
        ax.set_title('Proporsi Cluster (%)', color='white')
        st.pyplot(fig)
        plt.close(fig)

    with col_top:
        st.markdown("<div class='section-title'>🏆 Top 10 Terlaris</div>", unsafe_allow_html=True)
        st.caption("Daftar 10 produk dengan total kuantitas penjualan tertinggi secara keseluruhan.")
        ...
        ax.invert_yaxis()
        ax.set_title('10 Produk Terlaris', color='white')
        ax.tick_params(colors='#b0c4de')
        st.pyplot(fig)
        plt.close(fig)
